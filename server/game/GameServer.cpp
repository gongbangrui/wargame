...
bool isDemoProfile(const QString& profile) {
    return profile == QLatin1String("vmf-demo-v2");
}

bool isSingleSideVmfProfile(const QString& profile) {
    return profile == QLatin1String("vmf-guided-strike-v1") || isDemoProfile(profile);
}
...
QString validateNetworkScenario(const Scenario& scenario, bool requireCompleteRoster = true,
    ...
    const QSet<QString> knownKinds{QStringLiteral("commandpost"), QStringLiteral("reconuav"),
                                   ...
                                   QStringLiteral("jammeruav"), QStringLiteral("groundtarget")};
    for (const ScenarioUnit& unit : scenario.units) {
...
GameServer::GameServer(QObject* parent)
    ...
    if (!m_authoritativeRoom.setScenarioUnits(ScenarioIo::defaultScenario().units,
                                              &templateError)) {
        m_recoveryError = templateError;
        return;
    }
    ...
    const auto bootstrapRoomState = [this]() {
        ...
        if (!m_authoritativeRoom.setScenarioUnits(m_runInitialScenario.units,
                                                  &checkpointError)) {
            m_recoveryError = checkpointError;
            qCritical() << "默认场景单位绑定无效:" << checkpointError;
            return;
        }
...
void GameServer::refreshRoomControlForJoin(QWebSocket* socket, const QJsonObject& payload,
    ...
    connect(reply, &QNetworkReply::finished, this,
        ...
        if (selected.isEmpty()) {
            ...
            const QString selectedProfile = selected.value(QStringLiteral("protocolProfile"))
                                                .toString(QStringLiteral("native"));
            const ClientSession negotiated = m_clients.value(guardedSocket.data());
            if (isSingleSideVmfProfile(selectedProfile)
                && (negotiated.protocolVersion != Protocol::Version
                    || negotiated.schemaVersion != Protocol::SchemaVersion)) {
                sendError(guardedSocket, QStringLiteral("PROTOCOL_UPGRADE_REQUIRED"),
                          QStringLiteral("严格 VMF 房间要求协议 v8/schema 8"));
                return;
            }
            ...
            if (payload.value(QStringLiteral("asObserver")).toBool()) {
                ...
                m_protocolProfile = selectedProfile;
            ...
            const QString configuredProfile = selectedProfile;
            if (configuredProfile != QLatin1String("native")
                && !isSingleSideVmfProfile(configuredProfile)) {
                sendError(guardedSocket, QStringLiteral("ROOM_CONFIG_INVALID"),
                          QStringLiteral("房间协议 profile 无效"));
                return;
            }
            if (m_phase == QLatin1String("preparing")) {
                ...
                const RoomStateBackup profileBackup = captureRoomState();
                m_protocolProfile = configuredProfile;
                QString profileError;
                const bool profileApplied = applyProtocolProfilePolicy(&profileError);
                if (!profileApplied) {
                    restoreRoomStateBackup(profileBackup);
                    sendError(guardedSocket, QStringLiteral("ROOM_CONFIG_INVALID"),
                              profileError);
                    return;
                }
...
void GameServer::onTextMessage(QWebSocket* socket, const QString& text) {
    ...
    if (type == QLatin1String("vmfTaskCommand")) {
        ...
        const QString cacheKey = QStringLiteral("vmf-task:%1:%2")
                                     .arg(session.userId).arg(requestId);
        if (m_commandResults.contains(cacheKey)) {
            sendEnvelope(socket, QString::fromLatin1(Protocol::VmfTaskResultMessage),
                         m_commandResults.value(cacheKey)
                             .value(QStringLiteral("vmfTaskResult")).toObject());
            return;
        }
    }
    if (type == QLatin1String("demoAction") || type == QLatin1String("demoControl")) {
        const QString requestId = payload.value(QStringLiteral("requestId")).toString();
        const QString cacheKey = QStringLiteral("demo:%1:%2")
                                     .arg(session.userId).arg(requestId);
        if (m_commandResults.contains(cacheKey)) {
            sendEnvelope(socket, QString::fromLatin1(Protocol::DemoResultMessage),
                         m_commandResults.value(cacheKey)
                             .value(QStringLiteral("demoResult")).toObject());
            return;
        }
    }
    if (session.recentMessageIds.contains(messageId)) {
    ...
    else if (type == QLatin1String("vmfTaskCommand")) handleVmfTaskCommand(socket, payload, messageId);
    else if (type == QLatin1String("demoAction")) handleDemoAction(socket, payload, messageId);
    else if (type == QLatin1String("demoControl")) handleDemoControl(socket, payload, messageId);
...
void GameServer::syncRoomControl(QWebSocket* requester) {
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        ...
        const QString configuredDifficulty = aiDifficultyFromConfig(selected);
        const quint64 configuredVersion = configVersionFromConfig(selected);
        const QString configuredProfile = selected.value(QStringLiteral("protocolProfile"))
                                              .toString(QStringLiteral("native"));
        if (configuredProfile != QLatin1String("native")
            && !isSingleSideVmfProfile(configuredProfile)) {
            audit(QStringLiteral("security"),
                  QJsonObject{{QStringLiteral("event"),
                               QStringLiteral("protocolProfileChangeRejected")},
                              {QStringLiteral("profile"), configuredProfile},
                              {QStringLiteral("reason"), QStringLiteral("INVALID_PROFILE")}});
            flushRoomListWaiters();
            return;
        }
        if (m_phase == QLatin1String("preparing")) {
            ...
            m_protocolProfile = configuredProfile;
            QString profileError;
            if (!applyProtocolProfilePolicy(&profileError)) {
                restoreRoomStateBackup(profileBackup);
                audit(QStringLiteral("security"),
                      QJsonObject{{QStringLiteral("event"),
                                  QStringLiteral("protocolProfileChangeRejected")},
        ...
        if (status == QLatin1String("running") && m_phase != QLatin1String("running")) {
            if (isSingleSideVmfProfile(m_protocolProfile)) {
                QString vmfError;
                if (!applyProtocolProfilePolicy(&vmfError)) {
                    m_roomStatus = QStringLiteral("preparing");
                    reportRoomStatus(QStringLiteral("preparing"), vmfError);
                    audit(QStringLiteral("security"),
                          QJsonObject{{QStringLiteral("event"),
                                       QStringLiteral("adminStartRejected")},
                                      {QStringLiteral("reason"), vmfError}});
                    flushRoomListWaiters();
                    return;
                }
            }
...
void GameServer::sendSeatDirectory(QWebSocket* socket) {
    ...
    for (auto it = m_seatLimits.cbegin(); it != m_seatLimits.cend(); ++it) {
        ...
        for (int index = 1; index <= seatCount; ++index) {
            ...
            projectedSeatIds.insert(seatId);
            ...
            const bool placeholder = authoritative.controllerType == QLatin1String("placeholder");
            const bool occupied = ai || placeholder || occupant.userId > 0;
            const bool singleSideVmf = isSingleSideVmfProfile(m_protocolProfile);
            const bool claimable = singleSideVmf
                ? descriptor.side == QLatin1String("red") && (!occupied || placeholder)
                : !occupied;
            seats.append(QJsonObject{{QStringLiteral("seatId"), seatId},
                                     ...
                                     {QStringLiteral("displayName"),
                                      ai ? QStringLiteral("AI")
                                         : placeholder ? authoritative.username
                                                       : occupant.username},
                                     {QStringLiteral("connected"), ai || placeholder || authoritative.connected},
                                     {QStringLiteral("commander"), descriptor.type == QLatin1String("commander")},
                                     {QStringLiteral("controllerType"),
                                      ai ? QStringLiteral("ai") : placeholder ? QStringLiteral("placeholder")
                                                                     : QStringLiteral("human")},
                                     {QStringLiteral("controlMode"), authoritative.controlMode},
                                     {QStringLiteral("claimable"), claimable},
                                     {QStringLiteral("selectedTemplate"), authoritative.selectedTemplate},
                                     {QStringLiteral("sourceUnitId"), authoritative.sourceUnitId},
                                     {QStringLiteral("unitId"), authoritative.unitId},
                                     {QStringLiteral("deployed"), authoritative.deployed},
                                     {QStringLiteral("ready"), authoritative.ready},
    ...
    for (const AuthoritativeRoom::Seat& authoritative : m_authoritativeRoom.seats()) {
        if (projectedSeatIds.contains(authoritative.seatId)) continue;
        const bool placeholder =
            authoritative.controllerType == QLatin1String("placeholder");
        seats.append(QJsonObject{
            {QStringLiteral("seatId"), authoritative.seatId},
            {QStringLiteral("seatType"), authoritative.seatType},
            {QStringLiteral("side"), authoritative.side},
            {QStringLiteral("slot"), 1},
            {QStringLiteral("capacity"), 1},
            {QStringLiteral("occupied"), true},
            {QStringLiteral("displayName"), authoritative.username},
            {QStringLiteral("connected"), authoritative.connected},
            {QStringLiteral("commander"),
             authoritative.seatType == QLatin1String("commander")},
            {QStringLiteral("controllerType"), authoritative.controllerType},
            {QStringLiteral("controlMode"), authoritative.controlMode},
            {QStringLiteral("claimable"),
             isSingleSideVmfProfile(m_protocolProfile)
                 && authoritative.side == QLatin1String("red") && placeholder},
            {QStringLiteral("selectedTemplate"), authoritative.selectedTemplate},
            {QStringLiteral("sourceUnitId"), authoritative.sourceUnitId},
            {QStringLiteral("unitId"), authoritative.unitId},
            {QStringLiteral("deployed"), authoritative.deployed},
            ...
            {QStringLiteral("redeployRequested"), authoritative.redeployRequested},
            {QStringLiteral("unitName"), authoritative.unitName},
            {QStringLiteral("state"), QStringLiteral("occupied")},
            {QStringLiteral("revision"), static_cast<qint64>(authoritative.revision)}});
    }
...
void GameServer::handleClaimSeat(QWebSocket* socket, const QJsonObject& payload) {
    ...
    const QString requestedSeatId = requestedTakeoverSeatId;
    ...
    if (isSingleSideVmfProfile(m_protocolProfile)
        && side != QLatin1String("red")) {
        sendError(socket, QStringLiteral("SIDE_RESERVED_FOR_FIXED_TARGET"),
                  QStringLiteral("严格 VMF 房间只允许选择红方战位"));
        return;
    }
    ...
    syncAuthoritativeSeats();
    QString deploymentError;
    if (!livePlaceholderTakeover && !applyDeployedScenario(&deploymentError)) {
        restoreRoomStateBackup(backup);
...
void GameServer::handleRoomConfigCommand(QWebSocket* socket, const QJsonObject& payload) {
    ...
    QJsonObject request{
        ...
        {QStringLiteral("protocol_profile"),
         args.value(QStringLiteral("protocolProfile")).toString(m_protocolProfile)},
    ...
    connect(reply, &QNetworkReply::finished, this,
        ...
        const QString nextProfile = room.value(QStringLiteral("protocolProfile"))
                                        .toString(QStringLiteral("native"));
        if (room.value(QStringLiteral("roomId")).toString() != m_roomId
            || nextVersion == 0 || limitsObject.isEmpty()
            || (nextProfile != QLatin1String("native")
                && !isSingleSideVmfProfile(nextProfile))) {
        ...
        m_protocolProfile = nextProfile;
        m_configVersion = nextVersion;
        m_lastRoomUpdate = room.value(QStringLiteral("updatedAt")).toString(m_lastRoomUpdate);
        m_seatParameters = nextParameters;
        QString deploymentError;
        if (!applyProtocolProfilePolicy(&deploymentError)) {
            QString rollbackError;
            restoreRoomStateBackup(backup, &rollbackError);
            finish(CommandResult::reject(
                QStringLiteral("ROOM_CONFIG_UPDATE_FAILED"), deploymentError));
            return;
        }
...
bool GameServer::prepareDemoMessage(const QJsonObject& payload,
                                    const QString& actorSeatId,
                                    Message* message, QJsonObject* trace,
                                    QString* error) const {
    if (error) error->clear();
    if (!message || !trace) {
        if (error) *error = QStringLiteral("演示消息输出参数为空");
        return false;
    }
    const QString action = payload.value(QStringLiteral("action")).toString();
    ...
    if (action == QLatin1String("reportTarget")) {
    ...
    } else if (action == QLatin1String("planRoute")) {
        type = Message::Type::FlightPlan;
        receiverSeatId = QStringLiteral("red_attack_1");
    } else if (action == QLatin1String("acceptRoute")) {
        type = Message::Type::RouteAcceptance;
        receiverSeatId = QStringLiteral("red_commander");
    } else if (action == QLatin1String("issueGuidance")) {
        ...
        if (error) *error = QStringLiteral("演示动作没有 VMF 消息映射");
        return false;
    }

    const AuthoritativeRoom::Seat senderSeat = m_authoritativeRoom.seat(actorSeatId);
    const AuthoritativeRoom::Seat receiverSeat = m_authoritativeRoom.seat(receiverSeatId);
    if (senderSeat.unitId.isEmpty() || receiverSeat.unitId.isEmpty()) {
        if (error) *error = QStringLiteral("演示动作所需红方战位尚未配置");
        return false;
    }
    const UnitBase* sender = m_engine.unit(senderSeat.unitId);
    const UnitBase* receiver = m_engine.unit(receiverSeat.unitId);
    if (!sender || !receiver) {
        if (error) *error = QStringLiteral("演示动作所需红方单位尚未部署");
        return false;
    }
    ...
    if (action == QLatin1String("reportTarget") && !target && !positionTarget) {
        ...
        ids.sort();
        target = nullptr;
        const UnitBase* groundFallback = nullptr;
        const UnitBase* sideFallback = nullptr;
        for (const QString& id : ids) {
            const UnitBase* candidate = m_engine.unit(id);
            if (!candidate || candidate->sideStr() != QLatin1String("blue")) continue;
            if (!sideFallback) sideFallback = candidate;
            if (!groundFallback && candidate->kind() == UnitKind::GroundScout) {
                groundFallback = candidate;
            }
            if (candidate->kind() == UnitKind::GroundTarget) {
                target = candidate;
                break;
            }
        }
    ...
    Message source;
    source.id = QStringLiteral("demo-msg-%1").arg(
        payload.value(QStringLiteral("actionId")).toString());
    source.traceId = QStringLiteral("demo-trace-%1").arg(
        payload.value(QStringLiteral("actionId")).toString());
    source.correlationId = QStringLiteral("demo-correlation-%1").arg(
    ...
    const bool encodedOk = inputMode == QLatin1String("xml")
        ? m_engine.prepareVmfXmlMessage(
              vmf::VmfMessageGateway::messageNameForType(source.type, source.payload),
              payload.value(QStringLiteral("payload")).toObject()
                  .value(QStringLiteral("xml")).toString().toUtf8(),
              source, &encoded, trace, &encodeError)
        : m_engine.prepareVmfMessageWithTrace(source, &encoded, trace, &encodeError);
    if (!encodedOk) {
    ...
    trace->insert(QStringLiteral("plainText"), QJsonObject{
        ...
        {QStringLiteral("receiver"), receiver->id()},
        {QStringLiteral("targetId"), targetId},
        {QStringLiteral("fields"), data}});
    *message = std::move(encoded);
    return true;
}

bool GameServer::executeDemoAction(const QJsonObject& payload,
                                   const QString& actorSeatType,
                                   const QString& actorSeatId, bool automatic,
                                   QJsonObject* resultObject, QJsonObject* trace,
                                   QString* error) {
    if (error) error->clear();
    const QString actionActor = automatic ? QStringLiteral("automation") : actorSeatType;
    const QString requestId = payload.value(QStringLiteral("requestId")).toString();
    const QString cacheKey = automatic ? QString{}
        : QStringLiteral("demo:%1:%2")
              .arg(m_authoritativeRoom.seat(actorSeatId).userId)
              .arg(requestId);
    const auto responseFor = [&payload, &requestId](const VmfDemoWorkflow::Result& result) {
        return QJsonObject{{QStringLiteral("requestId"), requestId},
                           {QStringLiteral("status"), result.status},
                           {QStringLiteral("code"), result.code},
                           {QStringLiteral("message"), result.status == QLatin1String("duplicate")
                                ? QStringLiteral("演示动作已处理")
                                : QStringLiteral("演示动作已确认")},
                           {QStringLiteral("revision"), static_cast<qint64>(result.revision)},
                           {QStringLiteral("state"), result.state}};
    };
    const auto cacheResult = [this, &cacheKey, &requestId](const QJsonObject& response) {
        if (cacheKey.isEmpty()) return;
        const QJsonObject cached{{QStringLiteral("commandId"), requestId},
                                 {QStringLiteral("demoResult"), response}};
        if (!m_commandResults.contains(cacheKey)) m_commandResultOrder.append(cacheKey);
    ...
    if (const auto duplicate = m_demoWorkflow.duplicateActionResult(payload, actionActor)) {
        if (!duplicate->ok) {
            if (error) *error = duplicate->message;
            if (resultObject) {
                *resultObject = QJsonObject{{QStringLiteral("status"), duplicate->status},
                                            {QStringLiteral("code"), duplicate->code},
                                            {QStringLiteral("message"), duplicate->message},
                                            {QStringLiteral("revision"),
                                             static_cast<qint64>(duplicate->revision)}};
            }
            return false;
        }
        const QJsonObject response = responseFor(*duplicate);
    ...
    if (!result.ok) {
        ...
        if (resultObject) {
            *resultObject = QJsonObject{{QStringLiteral("status"), result.status},
                                        {QStringLiteral("code"), result.code},
                                        {QStringLiteral("message"), result.message},
                                        {QStringLiteral("revision"),
                                         static_cast<qint64>(result.revision)}};
        }
        return false;
    }
    const QJsonObject response = responseFor(result);
    QJsonObject durable{{QStringLiteral("demoState"), candidate.toJson()},
                        {QStringLiteral("demoResult"), response},
    ...
    QString persistenceError;
    if (!recordDurableEvent(QStringLiteral("demoAction"), durable, &persistenceError)) {
    ...
    validatedTrace.insert(QStringLiteral("actionId"),
                          payload.value(QStringLiteral("actionId")));
    if (resultObject) *resultObject = response;
    if (trace) *trace = validatedTrace;
    return true;
}

void GameServer::handleDemoAction(QWebSocket* socket, const QJsonObject& payload,
                                  const QString& requestId) {
    if (!m_clients.contains(socket)) return;
    const ClientSession session = m_clients.value(socket);
    const auto reject = [this, socket, requestId](const QString& code,
                                                  const QString& message) {
        sendEnvelope(socket, QString::fromLatin1(Protocol::DemoErrorMessage),
                     QJsonObject{{QStringLiteral("requestId"), requestId},
                                 {QStringLiteral("code"), code},
                                 {QStringLiteral("message"), message}});
    };
    if (!isDemoProfile(m_protocolProfile)) {
        reject(QStringLiteral("DEMO_PROFILE_REQUIRED"),
               QStringLiteral("当前房间未启用演示模式 v2"));
        return;
    }
    if (m_phase != QLatin1String("running") || !m_engine.running()) {
        reject(QStringLiteral("ROOM_NOT_RUNNING"),
               QStringLiteral("演示动作只能在推演运行阶段推进"));
        return;
    }
    if (session.roomId != m_roomId || session.observer || session.side != QLatin1String("red")
        || session.seatId.isEmpty()
        || payload.value(QStringLiteral("seat")).toString() != session.seatId) {
        reject(QStringLiteral("DEMO_SEAT_REQUIRED"),
               QStringLiteral("演示动作必须由当前红方战位提交"));
        return;
    }
    ...
    QJsonObject trace;
    QString actionError;
    if (!executeDemoAction(payload, session.seatType, session.seatId, false,
                           &result, &trace, &actionError)) {
        const QString code = result.value(QStringLiteral("code")).toString(
            QStringLiteral("DEMO_ACTION_REJECTED"));
        reject(code, actionError.isEmpty()
                         ? result.value(QStringLiteral("message")).toString()
                         : actionError);
        return;
    }
    sendEnvelope(socket, QString::fromLatin1(Protocol::DemoResultMessage), result);
    if (!trace.isEmpty()) broadcastDemoTrace(trace, socket);
    broadcastDemoState();
    broadcastSnapshots();
    persistRoomState();
}

void GameServer::handleDemoControl(QWebSocket* socket, const QJsonObject& payload,
                                   const QString& requestId) {
    if (!m_clients.contains(socket)) return;
    const ClientSession session = m_clients.value(socket);
    const auto reject = [this, socket, requestId](const QString& code,
    ...
    VmfDemoWorkflow candidate = m_demoWorkflow;
    const VmfDemoWorkflow::Result applied = candidate.applyControl(
        payload.value(QStringLiteral("action")).toString(),
        payload.value(QStringLiteral("payload")).toObject(), m_engine.simTime());
    if (!applied.ok) {
        reject(applied.code, applied.message);
        return;
    }
    const QJsonObject response{{QStringLiteral("requestId"), requestId},
                               {QStringLiteral("status"), applied.status},
                               {QStringLiteral("code"), applied.code},
                               {QStringLiteral("message"), QStringLiteral("演示控制已确认")},
                               {QStringLiteral("revision"),
                                static_cast<qint64>(applied.revision)},
                               {QStringLiteral("state"), applied.state}};
    const QString cacheKey = QStringLiteral("demo:%1:%2")
        .arg(session.userId).arg(requestId);
    const QJsonObject durable{{QStringLiteral("demoState"), candidate.toJson()},
                              {QStringLiteral("demoResult"), response},
                              {QStringLiteral("cacheKey"), cacheKey},
                              {QStringLiteral("requestId"), requestId}};
    QString persistenceError;
    if (!recordDurableEvent(QStringLiteral("demoControl"), durable,
                            &persistenceError)) {
        reject(QStringLiteral("PERSISTENCE_FAILED"), persistenceError);
        return;
    }
    ...
    m_demoNextAutomaticAt = m_engine.simTime() + 1.0;
    if (!m_commandResults.contains(cacheKey)) m_commandResultOrder.append(cacheKey);
    m_commandResults.insert(cacheKey,
    ...
    ++m_stateRevision;
    sendEnvelope(socket, QString::fromLatin1(Protocol::DemoResultMessage), response);
    broadcastDemoState();
    broadcastSnapshots(true);
    persistRoomState();
}

void GameServer::runDemoAutomation() {
    if (m_replayingDurableEvents || !isDemoProfile(m_protocolProfile)
        || m_phase != QLatin1String("running") || !m_engine.running()) return;
    const double now = m_engine.simTime();
    if (now < m_demoNextAutomaticAt) return;
    const VmfDemoWorkflow::ActionSpec* spec = m_demoWorkflow.currentAction();
    if (!spec) return;
    ...
    const QString actionId = QStringLiteral("auto-%1-%2")
        ...
        .arg(state.value(QStringLiteral("revision")).toInteger());
    const QJsonObject command{{QStringLiteral("requestId"), actionId},
    ...
    QJsonObject trace;
    QString error;
    if (!executeDemoAction(command, spec->seatType, seatId, true,
                           &result, &trace, &error)) {
        audit(QStringLiteral("demo"), QJsonObject{
            {QStringLiteral("event"), QStringLiteral("automationRejected")},
            {QStringLiteral("action"), spec->action},
            {QStringLiteral("message"), error}});
        m_demoNextAutomaticAt = now + 1.0;
        return;
    }
    m_demoNextAutomaticAt = now + 1.0;
    if (!trace.isEmpty()) broadcastDemoTrace(trace);
    broadcastDemoState();
    broadcastSnapshots();
    persistRoomState();
}

void GameServer::broadcastDemoState() {
    for (auto it = m_clients.begin(); it != m_clients.end(); ++it) {
        if (!it->authenticated || it->roomId != m_roomId) continue;
        sendEnvelope(it.key(), QString::fromLatin1(Protocol::DemoStateMessage),
                     m_demoWorkflow.stateProjection(false));
    }
}

void GameServer::broadcastDemoTrace(const QJsonObject& trace, QWebSocket* initiator) {
    for (auto it = m_clients.begin(); it != m_clients.end(); ++it) {
        if (!it->authenticated || it->roomId != m_roomId) continue;
        const bool privileged = it->accountRole == QLatin1String("room_admin")
            || it->role == QLatin1String("director");
        const bool permitted = it->side == QLatin1String("red") || privileged;
        if (it->observer && !privileged) continue;
        if (!permitted || (initiator && it.key() != initiator
                           && it->side != QLatin1String("red")
                           && !privileged)) continue;
        sendEnvelope(it.key(), QString::fromLatin1(Protocol::DemoTraceMessage), trace);
    }
}

void GameServer::handleVmfTaskCommand(QWebSocket* socket, const QJsonObject& payload,
                                      const QString& requestId) {
    if (!m_clients.contains(socket)) return;
    ClientSession& session = m_clients[socket];
    auto reject = [&](const QString& code, const QString& message) {
        sendError(socket, code, message, requestId);
    };
    if (m_protocolProfile != QLatin1String("vmf-guided-strike-v1")) {
        reject(QStringLiteral("VMF_TASK_PROFILE_REQUIRED"),
               QStringLiteral("当前房间未启用严格 VMF 任务 profile"));
        return;
    }
    if (session.roomId != m_roomId || session.seatId.isEmpty() || session.observer) {
        reject(QStringLiteral("SEAT_REQUIRED"), QStringLiteral("严格 VMF 任务需要可用战位"));
        return;
    }
    if (m_phase != QLatin1String("running") || !m_engine.running()) {
        reject(QStringLiteral("ROOM_NOT_RUNNING"),
               QStringLiteral("严格 VMF 任务只能在推演运行阶段推进"));
        return;
    }
    const QString action = payload.value(QStringLiteral("action")).toString();
    const double now = m_engine.simTime();
    const QString cacheKey = QStringLiteral("vmf-task:%1:%2")
                                 .arg(session.userId).arg(requestId);
    const auto cacheTaskResult = [this, &cacheKey, &requestId](const QJsonObject& response) {
        const QJsonObject cached{{QStringLiteral("commandId"), requestId},
    ...
    StrictVmfTaskSet candidate = m_strictVmfTasks;
    StrictVmfTaskSet::Result result;
    if (action == QLatin1String("createTask")) {
        if (session.seatType != QLatin1String("commander")) {
            reject(QStringLiteral("VMF_ROLE_FORBIDDEN"), QStringLiteral("只有指挥席可以创建任务"));
            return;
        }
        QSet<QString> placeholders;
        for (const auto& seat : m_authoritativeRoom.seats()) {
            if (seat.controlMode == QLatin1String("fixed-target")) placeholders.insert(seat.seatId);
        }
        const QStringList bindings{payload.value(QStringLiteral("commanderSeatId")).toString(),
                                   payload.value(QStringLiteral("reconSeatId")).toString(),
                                   payload.value(QStringLiteral("attackSeatId")).toString(),
                                   payload.value(QStringLiteral("groundSeatId")).toString()};
        if (bindings.first() != session.seatId) {
            reject(QStringLiteral("VMF_ROLE_FORBIDDEN"), QStringLiteral("任务指挥席必须是当前席位"));
            return;
        }
        const QStringList expectedTypes{QStringLiteral("commander"), QStringLiteral("recon"),
                                        QStringLiteral("attack"), QStringLiteral("ground")};
        for (qsizetype index = 0; index < bindings.size(); ++index) {
            if (!m_authoritativeRoom.hasSeat(bindings.at(index))) {
                reject(QStringLiteral("INVALID_TASK"),
                       QStringLiteral("任务绑定战位不存在"));
                return;
            }
            const AuthoritativeRoom::Seat bound = m_authoritativeRoom.seat(bindings.at(index));
            if (bound.side != session.side || bound.seatType != expectedTypes.at(index)
                || bound.unitId.isEmpty() || !m_engine.unit(bound.unitId)) {
                reject(QStringLiteral("INVALID_TASK"),
                       QStringLiteral("任务绑定战位、角色或运行单元不一致"));
                return;
            }
        }
        const UnitBase* target = m_engine.unit(
            payload.value(QStringLiteral("targetId")).toString());
        if (!target || !target->alive() || target->sideStr() == session.side) {
            reject(QStringLiteral("INVALID_TARGET"),
                   QStringLiteral("严格 VMF 任务必须绑定敌方已观测目标"));
            return;
        }
        if (!visibleUnitIds(session).contains(target->id())) {
            reject(QStringLiteral("TARGET_NOT_OBSERVED"),
                   QStringLiteral("严格 VMF 任务只能绑定本方已观测目标"));
            return;
        }
        bool waiting = false;
        ...
        QJsonObject authoritativeCommand = payload;
        if (action == QLatin1String("confirmDamageAssessment")) {
            const StrictVmfTaskSet::Task* assessed = m_strictVmfTasks.task(
                payload.value(QStringLiteral("taskId")).toString());
            const UnitBase* target = assessed ? m_engine.unit(assessed->targetId) : nullptr;
            authoritativeCommand.insert(QStringLiteral("targetDestroyed"),
                                        target && !target->alive());
        }
        result = candidate.applyAction(authoritativeCommand, session.seatId,
                                       placeholders, now);
    }
    if (!result.ok) {
        QJsonObject response{{QStringLiteral("status"), result.status},
                             {QStringLiteral("taskRevision"), static_cast<qint64>(result.taskRevision)},
                             {QStringLiteral("messageIds"), QJsonArray::fromStringList(result.messageIds)},
                             {QStringLiteral("code"), result.code},
                             {QStringLiteral("retryable"), false},
                             {QStringLiteral("requestId"), requestId}};
        cacheTaskResult(response);
        sendEnvelope(socket, QString::fromLatin1(Protocol::VmfTaskResultMessage), response);
        return;
    }
    QStringList validatedMessageIds;
    QJsonArray validatedTraces;
    const StrictVmfTaskSet::Task* currentTask = m_strictVmfTasks.task(
        payload.value(QStringLiteral("taskId")).toString());
    if (action != QLatin1String("createTask")) {
        if (!currentTask) {
            reject(QStringLiteral("TASK_NOT_FOUND"), QStringLiteral("任务不存在"));
            return;
        }
        QString senderSeatId;
        QString receiverSeatId;
        QString expectedDomainType;
        bool requiresMessage = true;
        if (action == QLatin1String("reportTarget")) {
            senderSeatId = currentTask->reconSeatId;
            receiverSeatId = currentTask->commanderSeatId;
            expectedDomainType = QStringLiteral("TargetReport");
        } else if (action == QLatin1String("dispatch")) {
            senderSeatId = currentTask->commanderSeatId;
            receiverSeatId = currentTask->attackSeatId;
            expectedDomainType = QStringLiteral("AttackOrder");
        } else if (action == QLatin1String("acceptDispatch")
                   || action == QLatin1String("acceptGuidance")) {
            senderSeatId = currentTask->attackSeatId;
            receiverSeatId = action == QLatin1String("acceptDispatch")
                ? currentTask->commanderSeatId : currentTask->groundSeatId;
            expectedDomainType = QStringLiteral("RouteAcceptance");
        } else if (action == QLatin1String("orderGroundGuidance")) {
            senderSeatId = currentTask->commanderSeatId;
            receiverSeatId = currentTask->groundSeatId;
            expectedDomainType = QStringLiteral("GroundGuideOrder");
        } else if (action == QLatin1String("identityHello")
                   || action == QLatin1String("identityConfirm")) {
            senderSeatId = action == QLatin1String("identityHello")
                ? currentTask->attackSeatId : currentTask->groundSeatId;
            receiverSeatId = action == QLatin1String("identityHello")
                ? currentTask->groundSeatId : currentTask->attackSeatId;
            expectedDomainType = QStringLiteral("IdentityReport");
        } else if (action == QLatin1String("sendGuidancePackage")) {
            senderSeatId = currentTask->groundSeatId;
            receiverSeatId = currentTask->attackSeatId;
            expectedDomainType = QStringLiteral("GroundTargetReport");
        } else if (action == QLatin1String("reportAttackReady")) {
            senderSeatId = currentTask->attackSeatId;
            receiverSeatId = currentTask->groundSeatId;
            expectedDomainType = QStringLiteral("AttackReadyReport");
        } else if (action == QLatin1String("authorizeAttack")) {
            senderSeatId = currentTask->groundSeatId;
            receiverSeatId = currentTask->attackSeatId;
            expectedDomainType = QStringLiteral("AttackAuthorization");
        } else if (action == QLatin1String("reportBattleDamage")) {
            senderSeatId = currentTask->attackSeatId;
            receiverSeatId = currentTask->groundSeatId;
            expectedDomainType = QStringLiteral("BattleDamageReport");
        } else if (action == QLatin1String("confirmDamageAssessment")) {
            senderSeatId = currentTask->groundSeatId;
            receiverSeatId = currentTask->attackSeatId;
            expectedDomainType = QStringLiteral("DamageAssessmentConfirm");
        } else if (action == QLatin1String("confirmTargetDestroyed")) {
            senderSeatId = currentTask->reconSeatId;
            receiverSeatId = currentTask->commanderSeatId;
            expectedDomainType = QStringLiteral("TargetDestroyed");
        } else if (action == QLatin1String("withdraw")) {
            senderSeatId = currentTask->commanderSeatId;
            receiverSeatId = currentTask->attackSeatId;
            expectedDomainType = QStringLiteral("WithdrawOrder");
        } else {
            requiresMessage = false;
        }

        const QJsonArray messages = payload.value(QStringLiteral("messages")).toArray();
        if ((requiresMessage && messages.size() != 1)
            || (!requiresMessage && !messages.isEmpty())) {
            reject(QStringLiteral("VMF_MESSAGE_SET_INVALID"),
                   QStringLiteral("当前任务动作的 VMF 消息集合不完整"));
            return;
        }
        if (requiresMessage) {
            const AuthoritativeRoom::Seat senderSeat = m_authoritativeRoom.seat(senderSeatId);
            const AuthoritativeRoom::Seat receiverSeat = m_authoritativeRoom.seat(receiverSeatId);
            const QJsonObject wire = messages.first().toObject();
            Message::Type type;
            const QByteArray encodedText = wire.value(QStringLiteral("wireBytes"))
                                               .toString().toLatin1();
            const QByteArray encodedBytes = QByteArray::fromBase64(encodedText);
            const QDateTime timestamp = QDateTime::fromString(
                wire.value(QStringLiteral("timestamp")).toString(), Qt::ISODateWithMs);
            if (senderSeat.unitId.isEmpty() || receiverSeat.unitId.isEmpty()
                || wire.value(QStringLiteral("domainType")).toString() != expectedDomainType
                || !Message::parseTypeName(expectedDomainType, &type)
                || encodedBytes.isEmpty() || encodedBytes.toBase64() != encodedText
                || !timestamp.isValid()
                || wire.value(QStringLiteral("correlationId")).toString()
                       != currentTask->correlationId
                || !m_engine.bus()
                || !m_engine.bus()->canCommunicate(senderSeat.unitId, receiverSeat.unitId)) {
                reject(QStringLiteral("VMF_MESSAGE_INVALID"),
                       QStringLiteral("VMF 消息身份、关联或通信链校验失败"));
                return;
            }
            QJsonObject domainPayload = wire.value(QStringLiteral("payload")).toObject();
            if (domainPayload.value(QStringLiteral("taskId")).toString()
                    != currentTask->taskId
                || domainPayload.value(QStringLiteral("targetId")).toString()
            ...
            if (action == QLatin1String("reportBattleDamage")
                ...
                if (!target) {
                    reject(QStringLiteral("INVALID_TARGET"), QStringLiteral("任务目标不存在"));
                    return;
                }
                domainPayload.insert(QStringLiteral("hp"), target->hp());
                domainPayload.insert(QStringLiteral("alive"), target->alive());
                domainPayload.insert(QStringLiteral("destroyed"), !target->alive());
            }
            Message supplied;
            supplied.id = wire.value(QStringLiteral("messageId")).toString();
            supplied.traceId = wire.value(QStringLiteral("traceId")).toString();
            supplied.correlationId = currentTask->correlationId;
            supplied.timestamp = timestamp;
            supplied.type = type;
            supplied.sender = senderSeat.unitId;
            supplied.receiver = receiverSeat.unitId;
            supplied.vmfMessage = wire.value(QStringLiteral("vmfMessage")).toString();
            supplied.wireFormat = Message::WireFormat::VmfDesignV1;
            supplied.wireBytes = encodedBytes;
            supplied.wireBitLength = wire.value(QStringLiteral("wireBitLength")).toInt();
            supplied.payload = domainPayload;
            QString validationError;
            if (!m_engine.validateVmfMessageForRoles(
                    supplied, senderSeat.seatType, receiverSeat.seatType, &validationError)) {
                reject(QStringLiteral("VMF_MESSAGE_INVALID"), validationError);
                return;
            }
            Message canonical;
            if (!m_engine.prepareVmfMessage(supplied, &canonical, &validationError)
                || canonical.wireBytes != supplied.wireBytes
                || canonical.wireBitLength != supplied.wireBitLength
                || canonical.vmfMessage != supplied.vmfMessage
                || canonical.payload.value(QStringLiteral("vmfCatalogId"))
                       != wire.value(QStringLiteral("catalogId"))) {
                reject(QStringLiteral("VMF_CANONICAL_MISMATCH"),
                       validationError.isEmpty()
                           ? QStringLiteral("客户端 VMF wire 与服务器规范编码不一致")
                           : validationError);
                return;
            }
            validatedMessageIds.append(supplied.id);
            validatedTraces.append(QJsonObject{
                {QStringLiteral("traceId"), supplied.traceId},
                {QStringLiteral("taskId"), currentTask->taskId},
                {QStringLiteral("messageId"), supplied.id},
                {QStringLiteral("vmfMessage"), supplied.vmfMessage},
                {QStringLiteral("catalogId"), wire.value(QStringLiteral("catalogId"))},
                {QStringLiteral("wireBitLength"), supplied.wireBitLength},
                {QStringLiteral("fieldCount"),
                 canonical.payload.value(QStringLiteral("vmfFieldCount"))},
                {QStringLiteral("roundTripEqual"), true}});
        }
        if (action == QLatin1String("confirmTargetDestroyed")) {
            const UnitBase* target = m_engine.unit(currentTask->targetId);
            if (!target || target->alive()) {
                reject(QStringLiteral("TARGET_NOT_DESTROYED"),
                       QStringLiteral("侦察确认不能先于权威毁伤结果"));
                return;
            }
        }
    }
    ...
    if (currentTask && action == QLatin1String("dispatch")) {
        ...
        engineCommand = {{QStringLiteral("attackerId"),
                         ...
                         {QStringLiteral("waypoints"), points.toVariantList()}};
    } else if (currentTask && action == QLatin1String("engage")) {
        engineAction = QStringLiteral("engageTarget");
        engineCommand = {{QStringLiteral("attackerId"),
                          m_authoritativeRoom.seat(currentTask->attackSeatId).unitId},
                         {QStringLiteral("targetId"), currentTask->targetId}};
    } else if (currentTask && action == QLatin1String("withdraw")) {
        const UnitBase* commander = m_engine.unit(
            m_authoritativeRoom.seat(currentTask->commanderSeatId).unitId);
        engineAction = QStringLiteral("withdraw");
        engineCommand = {{QStringLiteral("unitId"),
                          m_authoritativeRoom.seat(currentTask->attackSeatId).unitId},
                         {QStringLiteral("pos"), commander
                              ? QVariantMap{{QStringLiteral("x"), commander->pos().x},
                                            {QStringLiteral("y"), commander->pos().y}}
                              : QVariantMap{}}};
    }
    if (!engineAction.isEmpty()) {
        const CommandResult validation = m_engine.validateCommandRequest(engineAction,
                                                                          engineCommand);
    ...
    QJsonObject response{{QStringLiteral("status"), result.status},
                         {QStringLiteral("taskRevision"), static_cast<qint64>(result.taskRevision)},
                         {QStringLiteral("messageIds"), QJsonArray::fromStringList(validatedMessageIds)},
                         {QStringLiteral("code"), result.code},
                         {QStringLiteral("retryable"), result.status == QLatin1String("blocked")},
                         {QStringLiteral("requestId"), requestId}};
    QJsonObject durable = payload;
    durable.insert(QStringLiteral("userId"), session.userId);
    durable.insert(QStringLiteral("seatId"), session.seatId);
    durable.insert(QStringLiteral("status"), result.status);
    durable.insert(QStringLiteral("taskSet"), candidate.toJson());
    durable.insert(QStringLiteral("cacheKey"), cacheKey);
    durable.insert(QStringLiteral("taskResult"), response);
    if (!engineAction.isEmpty()) {
        durable.insert(QStringLiteral("engineAction"), engineAction);
        durable.insert(QStringLiteral("engineArgs"), QJsonObject::fromVariantMap(engineCommand));
    }
    QString persistenceError;
    if (!recordDurableEvent(QStringLiteral("vmfTaskCommand"), durable, &persistenceError)) {
        reject(QStringLiteral("PERSISTENCE_FAILED"), persistenceError);
        return;
    }
    ...
    cacheTaskResult(response);
    if (!engineAction.isEmpty()) {
        const CommandResult applied = m_engine.executeCommand(engineAction, engineCommand);
        if (!applied.accepted) {
            // The command was validated before the durable event was written.
            // A rejection here indicates an invariant breach (for example a
            // VMF encoder failure); stop advancing this room instead of
            // silently broadcasting a task state whose engine action vanished.
            m_engine.setRunning(false);
            m_phase = QStringLiteral("paused");
            m_roomStatus = QStringLiteral("paused");
            m_recoveryError = QStringLiteral("严格 VMF 引擎动作未应用: %1")
                                  .arg(applied.message);
            audit(QStringLiteral("recovery"), QJsonObject{
                {QStringLiteral("event"), QStringLiteral("vmfEngineDispatchFailed")},
                {QStringLiteral("taskId"), payload.value(QStringLiteral("taskId"))},
                {QStringLiteral("action"), engineAction},
                {QStringLiteral("message"), applied.message}});
            reject(QStringLiteral("VMF_ENGINE_DISPATCH_FAILED"), m_recoveryError);
            broadcastSnapshots(true);
            return;
        }
    }
    ++m_stateRevision;
    sendEnvelope(socket, QString::fromLatin1(Protocol::VmfTaskResultMessage), response);
    for (const QJsonValue& trace : validatedTraces) {
        sendEnvelope(socket, QString::fromLatin1(Protocol::VmfTraceMessage), trace.toObject());
    }
    broadcastSnapshots();
    persistRoomState();
}

void GameServer::runStrictVmfAutomation() {
    if (m_replayingDurableEvents
        || m_protocolProfile != QLatin1String("vmf-guided-strike-v1")
        || m_phase != QLatin1String("running") || !m_engine.running()) {
        return;
    }
    ...
    const auto actionForStage = [](const QString& stage) {
        ...
        return actions.value(stage);
    };
    const auto actorForAction = [](const StrictVmfTaskSet::Task& task, const QString& action) {
        if (action == QLatin1String("reportTarget")
            || action == QLatin1String("confirmTargetDestroyed")) return task.reconSeatId;
        if (action == QLatin1String("dispatch")
            || action == QLatin1String("orderGroundGuidance")
            || action == QLatin1String("withdraw")) return task.commanderSeatId;
        if (action == QLatin1String("acceptDispatch")
            || action == QLatin1String("identityHello")
            || action == QLatin1String("acceptGuidance")
            || action == QLatin1String("reportAttackReady")
            || action == QLatin1String("engage")
            || action == QLatin1String("reportBattleDamage")
            || action == QLatin1String("markReturning")) return task.attackSeatId;
        return task.groundSeatId;
    };
    const auto messageSpec = [](const StrictVmfTaskSet::Task& task, const QString& action,
                                Message::Type* type, QString* senderSeat,
                                QString* receiverSeat) {
        if (!type || !senderSeat || !receiverSeat) return false;
        if (action == QLatin1String("reportTarget")) {
            *type = Message::Type::TargetReport;
            *senderSeat = task.reconSeatId; *receiverSeat = task.commanderSeatId;
        } else if (action == QLatin1String("dispatch")) {
            *type = Message::Type::AttackOrder;
            *senderSeat = task.commanderSeatId; *receiverSeat = task.attackSeatId;
        } else if (action == QLatin1String("acceptDispatch")
                   || action == QLatin1String("acceptGuidance")) {
            *type = Message::Type::RouteAcceptance;
            *senderSeat = task.attackSeatId;
            *receiverSeat = action == QLatin1String("acceptDispatch")
                ? task.commanderSeatId : task.groundSeatId;
        } else if (action == QLatin1String("orderGroundGuidance")) {
            *type = Message::Type::GroundGuideOrder;
            *senderSeat = task.commanderSeatId; *receiverSeat = task.groundSeatId;
        } else if (action == QLatin1String("identityHello")
                   || action == QLatin1String("identityConfirm")) {
            *type = Message::Type::IdentityReport;
            *senderSeat = action == QLatin1String("identityHello")
                ? task.attackSeatId : task.groundSeatId;
            *receiverSeat = action == QLatin1String("identityHello")
                ? task.groundSeatId : task.attackSeatId;
        } else if (action == QLatin1String("sendGuidancePackage")) {
            *type = Message::Type::GroundTargetReport;
            *senderSeat = task.groundSeatId; *receiverSeat = task.attackSeatId;
        } else if (action == QLatin1String("reportAttackReady")) {
            *type = Message::Type::AttackReadyReport;
            *senderSeat = task.attackSeatId; *receiverSeat = task.groundSeatId;
        } else if (action == QLatin1String("authorizeAttack")) {
            *type = Message::Type::AttackAuthorization;
            *senderSeat = task.groundSeatId; *receiverSeat = task.attackSeatId;
        } else if (action == QLatin1String("reportBattleDamage")) {
    ...
    const double now = m_engine.simTime();
    for (const StrictVmfTaskSet::Task& task : m_strictVmfTasks.tasks()) {
        if (task.health == QLatin1String("completed")
            || task.health == QLatin1String("cancelled")) continue;
        const QString action = actionForStage(task.stage);
        if (action.isEmpty()) continue;
        const QString actorSeatId = actorForAction(task, action);
        const QString attemptKey = QStringLiteral("%1:%2:%3")
                                       .arg(task.taskId).arg(task.taskRevision).arg(action);
        if (now < m_vmfAutomationRetryAfter.value(attemptKey, 0.0)) continue;
        const auto backoff = [this, &attemptKey, now]() {
            const int failures = std::min(5, m_vmfAutomationFailureCount.value(attemptKey) + 1);
            m_vmfAutomationFailureCount.insert(attemptKey, failures);
            m_vmfAutomationRetryAfter.insert(
                attemptKey, now + 0.5 * static_cast<double>(1 << (failures - 1)));
        };
        const AuthoritativeRoom::Seat actorSeat = m_authoritativeRoom.seat(actorSeatId);
        if (actorSeat.seatId.isEmpty() || actorSeat.side != QLatin1String("red")
            || actorSeat.controlMode != QLatin1String("vmf-auto")) continue;
        const UnitBase* actor = m_engine.unit(actorSeat.unitId);
        const UnitBase* target = m_engine.unit(task.targetId);
        const bool requiresLivingTarget = action != QLatin1String("reportBattleDamage")
            && action != QLatin1String("confirmDamageAssessment")
        ...
        if (!actor || !actor->alive() || !target
            || (requiresLivingTarget && !target->alive())) {
            backoff();
            continue;
        }
        ...
        if ((task.stage == QLatin1String("damageReportPending")
             || task.stage == QLatin1String("damageAssessmentPending"))
            && now - task.updatedAt < 0.5) continue;
        if (action == QLatin1String("confirmTargetDestroyed")
            && (!target || target->alive())) continue;

        QJsonObject command{{QStringLiteral("taskId"), task.taskId},
                            {QStringLiteral("expectedTaskRevision"),
                             static_cast<qint64>(task.taskRevision)},
                            {QStringLiteral("action"), action}};
        if (action == QLatin1String("confirmDamageAssessment")) {
            command.insert(QStringLiteral("targetDestroyed"), target && !target->alive());
        }
        StrictVmfTaskSet candidate = m_strictVmfTasks;
        const StrictVmfTaskSet::Result result = candidate.applyAction(
            command, actorSeatId, {}, now);
        if (!result.ok) {
            backoff();
            continue;
        }
        ...
        QString senderSeatId;
        QString receiverSeatId;
        if (messageSpec(task, action, &messageType, &senderSeatId, &receiverSeatId)) {
            const AuthoritativeRoom::Seat senderSeat = m_authoritativeRoom.seat(senderSeatId);
            const AuthoritativeRoom::Seat receiverSeat = m_authoritativeRoom.seat(receiverSeatId);
            if (senderSeat.unitId.isEmpty() || receiverSeat.unitId.isEmpty()
                || !m_engine.bus()
            ...
            Message source;
            source.id = QStringLiteral("vmf-auto-%1-%2-%3")
                            .arg(task.taskId).arg(task.taskRevision).arg(action);
            source.traceId = source.id;
            source.correlationId = task.correlationId;
            source.type = messageType;
            source.sender = senderSeat.unitId;
            source.receiver = receiverSeat.unitId;
            source.timestamp = QDateTime::currentDateTimeUtc();
            source.payload = QJsonObject{{QStringLiteral("taskId"), task.taskId},
            ...
            if (target) {
                ...
                source.payload.insert(QStringLiteral("targets"), QJsonArray{
                    QJsonObject{{QStringLiteral("targetId"), task.targetId},
                                {QStringLiteral("targetType"), target->kindStr()},
                                {QStringLiteral("friendFoe"), QStringLiteral("enemy")},
                                {QStringLiteral("status"), target->alive() ? QStringLiteral("alive")
                                                                             : QStringLiteral("destroyed")},
                                {QStringLiteral("x"), p.x}, {QStringLiteral("y"), p.y}}});
            }
            if (action == QLatin1String("identityHello")
                || action == QLatin1String("identityConfirm")) {
                source.payload.insert(QStringLiteral("unitId"), senderSeat.unitId);
                source.payload.insert(QStringLiteral("time"), now);
            }
            if (action == QLatin1String("reportBattleDamage")
                || action == QLatin1String("confirmDamageAssessment")) {
                source.payload.insert(QStringLiteral("hp"), target ? target->hp() : 0.0);
                source.payload.insert(QStringLiteral("alive"), target && target->alive());
                source.payload.insert(QStringLiteral("destroyed"), !target || !target->alive());
                source.payload.insert(QStringLiteral("damage"), target ? target->maxHp() - target->hp() : 0.0);
            }
            Message encoded;
            QString encodeError;
            if (!m_engine.prepareVmfMessage(source, &encoded, &encodeError)) {
                audit(QStringLiteral("vmf"), QJsonObject{
                    {QStringLiteral("event"), QStringLiteral("automationEncodeFailed")},
                    {QStringLiteral("taskId"), task.taskId}, {QStringLiteral("action"), action},
                    {QStringLiteral("message"), encodeError}});
                backoff();
                continue;
            }
            QString roleError;
            if (!m_engine.validateVmfMessageForRoles(encoded, senderSeat.seatType,
                                                      receiverSeat.seatType, &roleError)) {
                audit(QStringLiteral("vmf"), QJsonObject{
                    {QStringLiteral("event"), QStringLiteral("automationRoleRejected")},
                    {QStringLiteral("taskId"), task.taskId}, {QStringLiteral("message"), roleError}});
                backoff();
                continue;
            }
            const QJsonObject wire{
                ...
                {QStringLiteral("catalogId"), encoded.payload.value(QStringLiteral("vmfCatalogId"))},
            ...
            traces.append(QJsonObject{{QStringLiteral("kind"), QStringLiteral("vmfMessage")},
                                      {QStringLiteral("traceId"), encoded.traceId},
                                      {QStringLiteral("taskId"), task.taskId},
                                      {QStringLiteral("messageId"), encoded.id},
                                      {QStringLiteral("vmfMessage"), encoded.vmfMessage},
        ...
        if (action == QLatin1String("dispatch")) {
            ...
            engineAction = QStringLiteral("withdraw");
            engineArgs = {{QStringLiteral("unitId"), m_authoritativeRoom.seat(task.attackSeatId).unitId},
                          {QStringLiteral("pos"), QVariantMap{{QStringLiteral("x"), commander->pos().x},
                                                                {QStringLiteral("y"), commander->pos().y}}}};
        }
        if (!engineAction.isEmpty()) {
            const CommandResult validation = m_engine.validateCommandRequest(engineAction, engineArgs);
            if (!validation.accepted) {
                backoff();
                continue;
            }
        ...
        if (!engineAction.isEmpty()) {
            durable.insert(QStringLiteral("engineAction"), engineAction);
            durable.insert(QStringLiteral("engineArgs"), QJsonObject::fromVariantMap(engineArgs));
        }
        QString persistenceError;
        if (!recordDurableEvent(QStringLiteral("vmfTaskCommand"), durable, &persistenceError)) {
            audit(QStringLiteral("persistence"), QJsonObject{
                {QStringLiteral("event"), QStringLiteral("automationTaskFailed")},
                {QStringLiteral("taskId"), task.taskId}, {QStringLiteral("message"), persistenceError}});
            backoff();
            continue;
        }
        m_strictVmfTasks = candidate;
        if (!engineAction.isEmpty()) {
            ...
            if (!applied.accepted) {
                // The event is already durable.  Fail closed and require the
                // normal recovery path rather than continuing with divergent
                // task and simulation state.
                m_engine.setRunning(false);
                m_phase = QStringLiteral("paused");
                m_roomStatus = QStringLiteral("paused");
                m_recoveryError = QStringLiteral("严格 VMF 自动动作未应用: %1")
                                      .arg(applied.message);
                audit(QStringLiteral("recovery"), QJsonObject{
                    {QStringLiteral("event"), QStringLiteral("vmfAutomationEngineDispatchFailed")},
                    {QStringLiteral("taskId"), durable.value(QStringLiteral("taskId"))},
                    {QStringLiteral("action"), engineAction},
                    {QStringLiteral("message"), applied.message}});
                broadcastSnapshots(true);
                return;
        ...
        m_vmfAutomationRetryAfter.remove(attemptKey);
...
void GameServer::completeStrictReturns() {
    if (m_protocolProfile != QLatin1String("vmf-guided-strike-v1")) return;
    StrictVmfTaskSet candidate = m_strictVmfTasks;
    bool changed = false;
    for (const StrictVmfTaskSet::Task& task : m_strictVmfTasks.tasks()) {
        if (task.stage != QLatin1String("returning")) continue;
        const UnitBase* attacker = m_engine.unit(
            m_authoritativeRoom.seat(task.attackSeatId).unitId);
        const UnitBase* commander = m_engine.unit(
            m_authoritativeRoom.seat(task.commanderSeatId).unitId);
        if (!attacker || !commander || !attacker->alive() || !commander->alive()
            || attacker->pos().distanceTo2D(commander->pos()) > 250.0) {
            continue;
        }
        const StrictVmfTaskSet::Result completed = candidate.completeReturn(
            task.taskId, m_engine.simTime());
        if (completed.ok) changed = true;
    }
    if (!changed) return;
    const QJsonObject durable{{QStringLiteral("action"), QStringLiteral("completeReturn")},
                              {QStringLiteral("taskSet"), candidate.toJson()}};
    QString error;
    if (!recordDurableEvent(QStringLiteral("vmfTaskCommand"), durable, &error)) {
        audit(QStringLiteral("persistence"),
              QJsonObject{{QStringLiteral("event"),
                           QStringLiteral("strictReturnCheckpointFailed")},
                          {QStringLiteral("message"), error}});
        return;
    }
    m_strictVmfTasks = candidate;
...
bool GameServer::applyInitialScenarioState(const Scenario& initialScenario,
    ...
    if (!m_authoritativeRoom.setScenarioUnits(initialScenario.units, &catalogError)) {
        if (error) *error = catalogError;
        return false;
    }
...
bool GameServer::replaceInitialScenario(const Scenario& scenario, QString* error) {
    ...
    if (isSingleSideVmfProfile(m_protocolProfile)
        && !applyProtocolProfilePolicy(&applyError)) {
        return rollback(QStringLiteral("VMF 单方策略应用失败: %1").arg(applyError));
    }
    ++m_stateRevision;
...
bool GameServer::persistRoomState(QString* error) {
    ...
    checkpoint.protocolProfile = m_protocolProfile;
    checkpoint.strictVmfTasks = m_strictVmfTasks.toJson();
    checkpoint.demoState = isDemoProfile(m_protocolProfile)
        ? m_demoWorkflow.toJson() : QJsonObject{};
    for (auto it = m_seatLimits.cbegin(); it != m_seatLimits.cend(); ++it) {
...
bool GameServer::restoreRoomState(QString* error) {
    ...
    if (isSingleSideVmfProfile(checkpoint.protocolProfile)) {
    ...
    m_seatLimits = seatLimitsForScenario(m_runInitialScenario);
    QString seatLimitError;
    if (!m_authoritativeRoom.setScenarioUnits(m_runInitialScenario.units, &seatLimitError)) {
        if (error) *error = seatLimitError;
        return false;
    }
    if (!m_authoritativeRoom.setSeatLimits(m_seatLimits, &seatLimitError)) {
    ...
    m_protocolProfile = checkpoint.protocolProfile.isEmpty()
        ? QStringLiteral("native") : checkpoint.protocolProfile;
    if (!m_strictVmfTasks.restore(checkpoint.strictVmfTasks, error)
        && !checkpoint.strictVmfTasks.isEmpty()) {
        return false;
    }
    if (isDemoProfile(m_protocolProfile)) {
        if (checkpoint.demoState.isEmpty()
            || !m_demoWorkflow.restore(checkpoint.demoState, error)) {
            if (error && error->isEmpty()) {
                *error = QStringLiteral("演示模式检查点状态无效");
            }
            return false;
        }
    } else {
    ...
    for (const QJsonValue& value : checkpoint.commandHistory) {
        ...
        QJsonObject result = entry.value(QStringLiteral("result")).toObject();
        if (result.value(QStringLiteral("commandId")).toString().isEmpty()) {
            const QString legacyDemoRequestId = result.value(QStringLiteral("demoResult"))
                                                    .toObject()
                                                    .value(QStringLiteral("requestId"))
                                                    .toString();
            if (!legacyDemoRequestId.isEmpty()) {
                result.insert(QStringLiteral("commandId"), legacyDemoRequestId);
            }
        }
    ...
    if (!checkpoint.authoritativeRoom.isEmpty()) {
        QJsonObject authoritativeRoom = checkpoint.authoritativeRoom;
        if (isSingleSideVmfProfile(m_protocolProfile)) {
            authoritativeRoom[QStringLiteral("operationMode")] =
                QStringLiteral("vmf-single-side");
        }
        if (!m_authoritativeRoom.restore(authoritativeRoom, error)) return false;
    }
    m_roomMode = m_authoritativeRoom.mode();
    m_engine.setSideFrozen(QStringLiteral("blue"),
                           isSingleSideVmfProfile(m_protocolProfile));
...
bool GameServer::applyDurableEvent(const QString& kind, const QJsonObject& payload,
                                   QString* error) {
    if (error) error->clear();
    if (kind == QLatin1String("demoAction") || kind == QLatin1String("demoControl")) {
        const QJsonObject state = payload.value(QStringLiteral("demoState")).toObject();
        VmfDemoWorkflow candidate = m_demoWorkflow;
        if (state.isEmpty() || !candidate.restore(state, error)) {
            if (error && error->isEmpty()) *error = QStringLiteral("演示模式事件状态无效");
            return false;
        }
        const QString cacheKey = payload.value(QStringLiteral("cacheKey")).toString();
        const QJsonObject result = payload.value(QStringLiteral("demoResult")).toObject();
        if (!cacheKey.isEmpty() && !result.isEmpty()) {
            const QString requestId = payload.value(QStringLiteral("requestId")).toString(
                result.value(QStringLiteral("requestId")).toString());
            if (requestId.isEmpty()) {
                if (error) *error = QStringLiteral("演示模式事件缺少请求 ID");
                return false;
            }
            if (!m_commandResults.contains(cacheKey)) m_commandResultOrder.append(cacheKey);
            m_commandResults.insert(cacheKey,
                                    QJsonObject{{QStringLiteral("commandId"), requestId},
                                                {QStringLiteral("demoResult"), result}});
            while (m_commandResultOrder.size() > 2048) {
                m_commandResults.remove(m_commandResultOrder.takeFirst());
            }
        }
    ...
    if (kind == QLatin1String("vmfTaskCommand")) {
        const QJsonObject taskSet = payload.value(QStringLiteral("taskSet")).toObject();
        StrictVmfTaskSet candidate = m_strictVmfTasks;
        if (taskSet.isEmpty() || !candidate.restore(taskSet, error)) {
            if (error && error->isEmpty()) *error = QStringLiteral("严格 VMF 任务事件无效");
            return false;
        }
        const QString cacheKey = payload.value(QStringLiteral("cacheKey")).toString();
        const QJsonObject taskResult = payload.value(QStringLiteral("taskResult")).toObject();
        const QString requestId = payload.value(QStringLiteral("requestId")).toString();
        const QString engineAction = payload.value(QStringLiteral("engineAction")).toString();
        if (!engineAction.isEmpty()) {
            const CommandResult applied = m_engine.executeCommand(
                engineAction, payload.value(QStringLiteral("engineArgs"))
                                  .toObject().toVariantMap());
            if (!applied.accepted) {
                if (error) {
                    *error = QStringLiteral("严格 VMF 引擎动作重放失败: %1")
                                 .arg(applied.message);
                }
                return false;
            }
        ...
        m_strictVmfTasks = candidate;
        if (!cacheKey.isEmpty() && !taskResult.isEmpty() && !requestId.isEmpty()) {
            const QJsonObject cached{{QStringLiteral("commandId"), requestId},
                                     {QStringLiteral("vmfTaskResult"), taskResult}};
            if (!m_commandResults.contains(cacheKey)) m_commandResultOrder.append(cacheKey);
            m_commandResults.insert(cacheKey, cached);
            while (m_commandResultOrder.size() > 2048) {
                m_commandResults.remove(m_commandResultOrder.takeFirst());
...
bool GameServer::applyDeployedScenario(QString* error) {
    ...
    if (isSingleSideVmfProfile(m_protocolProfile)) {
        ...
        for (const QJsonValue& value : units) {
            runtimeIds.insert(value.toObject().value(QStringLiteral("id")).toString());
        }
        for (const ScenarioUnit& unit : m_runInitialScenario.units) {
            if (unit.side != QLatin1String("blue") || runtimeIds.contains(unit.id)) continue;
            const QJsonArray encoded = ScenarioIo::toJson(
                Scenario{ScenarioMap{}, CommunicationPolicy{}, {unit}, {}})
                                           .value(QStringLiteral("units")).toArray();
            if (!encoded.isEmpty()) units.append(encoded.first());
            runtimeIds.insert(unit.id);
        }
    }
    ...
    appendStrictProfileTargets(&scenario);
    ...
    const bool unchanged = ScenarioIo::toJson(m_engine.scenario())
        == ScenarioIo::toJson(scenario);
    m_engine.setSideFrozen(QStringLiteral("blue"),
                           isSingleSideVmfProfile(m_protocolProfile));
...
void GameServer::appendStrictProfileTargets(Scenario* scenario) const {
    if (!scenario || !isSingleSideVmfProfile(m_protocolProfile)) return;
    scenario->communicationPolicy.format = QStringLiteral("vmf-design-v1");
    scenario->communicationPolicy.vmfProfile = isDemoProfile(m_protocolProfile)
        ? QString::fromLatin1(vmf::VmfProfile::DemoV2)
        : QString::fromLatin1(vmf::VmfProfile::DesignV1);
    scenario->communicationPolicy.automaticAck = true;
}
...
bool GameServer::applyProtocolProfilePolicy(QString* error) {
    ...
    if (m_protocolProfile != QLatin1String("vmf-guided-strike-v1")) {
        m_strictVmfTasks = {};
        m_vmfAutomationRetryAfter.clear();
        m_vmfAutomationFailureCount.clear();
    }
    if (isDemoProfile(m_protocolProfile) && m_phase == QLatin1String("preparing")) {
        m_demoWorkflow.reset(m_engine.simTime());
        m_demoNextAutomaticAt = m_engine.simTime() + 1.0;
    }
    m_engine.setSideFrozen(QStringLiteral("blue"), singleSide);
    if (m_phase == QLatin1String("preparing")) {
        QString deploymentError;
        if (!applyDeployedScenario(&deploymentError)) {
            if (error) *error = deploymentError;
            return false;
        }
    }
    syncAuthoritativeSeats();
...
GameServer::RoomStateBackup GameServer::captureRoomState() const {
    ...
    backup.protocolProfile = m_protocolProfile;
    backup.strictVmfTasks = m_strictVmfTasks.toJson();
    backup.demoState = m_demoWorkflow.toJson();
...
bool GameServer::restoreRoomStateBackup(const RoomStateBackup& backup, QString* error) {
    ...
    const Scenario initialScenario = backup.runInitialScenario.units.empty()
        ? backup.scenario : backup.runInitialScenario;
    if (!m_authoritativeRoom.setScenarioUnits(initialScenario.units, &localError)) {
        if (error) *error = localError;
        return false;
    }
    const QHash<QString, int> restoredSeatLimits = seatLimitsForScenario(initialScenario);
    if (!m_authoritativeRoom.setSeatLimits(restoredSeatLimits, &localError)) {
        if (error) *error = localError;
        return false;
    }
    ...
    m_protocolProfile = backup.protocolProfile.isEmpty()
        ? QStringLiteral("native") : backup.protocolProfile;
    m_engine.setSideFrozen(QStringLiteral("blue"),
                           isSingleSideVmfProfile(m_protocolProfile));
    if (!m_strictVmfTasks.restore(backup.strictVmfTasks, &localError)) {
        if (!backup.strictVmfTasks.isEmpty()) {
            if (error) *error = localError;
            return false;
        }
    }
    if (isDemoProfile(m_protocolProfile)) {
        if (!m_demoWorkflow.restore(backup.demoState, &localError)) {
            if (error) *error = localError;
            return false;
        }
    } else {
    ...
    m_seatLimits = restoredSeatLimits;
...
bool GameServer::resetAuthoritativeRuntime(const QString& operationId, QString* error) {
    ...
    if (committed) {
        m_phase = QStringLiteral("preparing");
        if (isSingleSideVmfProfile(m_protocolProfile)) {
            committed = applyProtocolProfilePolicy(&localError);
        } else {
            const AuthoritativeRoom::Result policy =
                m_authoritativeRoom.setVmfSingleSide(false);
            committed = policy.ok;
            if (!committed) localError = policy.code;
            m_engine.setSideFrozen(QStringLiteral("blue"), false);
            syncAuthoritativeSeats();
        }
    }
    if (committed) {
...
void GameServer::processRoomOperation(const QJsonObject& operation) {
    ...
    if (action == QLatin1String("start") || action == QLatin1String("resume")) {
        ...
        if (isSingleSideVmfProfile(m_protocolProfile)) {
            ...
            if (!applyProtocolProfilePolicy(&vmfError)) {
                audit(QStringLiteral("security"),
                      QJsonObject{{QStringLiteral("event"), QStringLiteral("vmfPolicyFailed")},
                                  {QStringLiteral("reason"), vmfError}});
                rollback(QStringLiteral("VMF_POLICY_FAILED"));
                return;
            }
        }
        if (m_roomMode == QLatin1String("pve")) {
...
QJsonObject GameServer::roomState() const {
    ...
    for (const AuthoritativeRoom::Seat& authoritative : m_authoritativeRoom.seats()) {
        if (projectedSeatIds.contains(authoritative.seatId)) continue;
        const bool placeholder = authoritative.controllerType == QLatin1String("placeholder");
        seats.append(QJsonObject{{QStringLiteral("seatId"), authoritative.seatId},
                                 {QStringLiteral("seatType"), authoritative.seatType},
                                 {QStringLiteral("side"), authoritative.side},
                                 {QStringLiteral("slot"), 1},
                                 {QStringLiteral("capacity"), 1},
                                 ...
                                 {QStringLiteral("controlMode"), authoritative.controlMode},
                                 {QStringLiteral("claimable"),
                                  placeholder && authoritative.side == QLatin1String("red")},
                                 {QStringLiteral("selectedTemplate"), authoritative.selectedTemplate},
                                 {QStringLiteral("sourceUnitId"), authoritative.sourceUnitId},
                                 {QStringLiteral("unitId"), authoritative.unitId},
                                 {QStringLiteral("deployed"), authoritative.deployed},
    ...
    QJsonObject state{{QStringLiteral("phase"), m_phase},
                       ...
                       {QStringLiteral("protocolProfile"), m_protocolProfile},
                       {QStringLiteral("operationMode"),
                        isSingleSideVmfProfile(m_protocolProfile)
                            ? QStringLiteral("vmf-single-side") : QStringLiteral("standard")},
                       {QStringLiteral("participantSide"),
                        isSingleSideVmfProfile(m_protocolProfile)
                            ? QStringLiteral("red") : QString()},
                       {QStringLiteral("fixedTargetSide"),
                        isSingleSideVmfProfile(m_protocolProfile)
                            ? QStringLiteral("blue") : QString()},
                       {QStringLiteral("scenarioEditable"),
                        m_phase == QLatin1String("preparing")
                            && (m_authoritativeRoom.seats().isEmpty()
                                || isSingleSideVmfProfile(m_protocolProfile))},
                       ...
                       {QStringLiteral("vmfTasks"),
                        m_protocolProfile == QLatin1String("vmf-guided-strike-v1")
                            ? m_strictVmfTasks.toJson() : QJsonObject{}}};
    if (isDemoProfile(m_protocolProfile)) {
        state.insert(QStringLiteral("demoState"), m_demoWorkflow.stateProjection(false));
    }
    return state;
}
...
QJsonObject GameServer::snapshotFor(const ClientSession& session, quint64 projectedRevision) const {
    ...
    if (m_protocolProfile == QLatin1String("vmf-guided-strike-v1") && !session.observer) {
        QJsonArray visibleTasks;
        const QJsonArray allTasks = m_strictVmfTasks.toJson().value(QStringLiteral("tasks")).toArray();
        for (const QJsonValue& value : allTasks) {
            const QJsonObject task = value.toObject();
            if (task.value(QStringLiteral("side")).toString() != session.side) continue;
            const bool commander = session.seatType == QLatin1String("commander");
            const bool bound = task.value(QStringLiteral("commanderSeatId")).toString() == session.seatId
                || task.value(QStringLiteral("reconSeatId")).toString() == session.seatId
                || task.value(QStringLiteral("attackSeatId")).toString() == session.seatId
                || task.value(QStringLiteral("groundSeatId")).toString() == session.seatId;
            if (commander || bound) visibleTasks.append(task);
        }
        projectedRoomState.insert(QStringLiteral("vmfTasks"), QJsonObject{
            {QStringLiteral("schemaVersion"), 1}, {QStringLiteral("tasks"), visibleTasks}});
    }
    if (isDemoProfile(m_protocolProfile)) {
        projectedRoomState.insert(QStringLiteral("demoState"),
                                  m_demoWorkflow.stateProjection(false));
    }
    if (session.observer) {
    ...
    // An empty runtime roster is expected before a participant has claimed a
    // seat.  Once a seat is confirmed, keep the projected scenario/runtime
    // intact so the client can render its owned initial unit even if a
    // recovery or deployment refresh has not populated every runtime entry.
    if (m_authoritativeRoom.runtimeUnits().isEmpty() && session.seatId.isEmpty()) {
