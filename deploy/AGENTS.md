...
- `package-one-click.sh` must emit a deterministic archive plus `.sha256` sidecar and exclude every `.env` variant, databases, JSONL, checkpoints, logs, backups, build outputs and other runtime secrets. `QUICK_START.md` is the package-local fresh-host guide.
