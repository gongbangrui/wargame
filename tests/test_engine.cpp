...
TEST_F(EngineTest, SharedDetectionRefreshesMovingTargetTrack) {
    auto* recon = engine.unit("red_r1");
    auto* ally = engine.unit("red_a1");
    auto* target = engine.unit("blue_g1");
    auto* commandPost = dynamic_cast<CommandPost*>(engine.unit("red_cp"));
    ASSERT_NE(recon, nullptr);
    ASSERT_NE(ally, nullptr);
    ASSERT_NE(target, nullptr);
    ...
    ally->setPosition(GeoPos{7000, 0, 0});
    target->setPosition(GeoPos{1000, 0, 0});
    recon->setDetectRange(3000.0);
    recon->setCommRange(10000.0);
    ally->setCommRange(10000.0);

    engine.stepOnce(1.5);
    target->setPosition(GeoPos{1300, 0, 0});
    engine.stepOnce(1.5);

    const QJsonObject shared = ally->sharedKnowledgeJson()
        .value("shared:detect:blue_g1").toObject();
    EXPECT_DOUBLE_EQ(shared.value("x").toDouble(), 1300.0);
    EXPECT_DOUBLE_EQ(commandPost->knownTarget("blue_g1").value("x").toDouble(), 1300.0);
}
