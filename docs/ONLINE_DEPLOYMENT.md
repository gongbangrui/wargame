...
当前联网数据面为协议 v7/schema 7。v7 客户端在 v6 情报分页和完整碰撞字段之上增加 VMF
工作流与 wire 摘要；服务端仍可与 v6/v4 客户端降级协商。发布前应协调替换客户端和 `game-server`，并备份 `/data/room-checkpoint.json`、
...
安装包由 `deploy/package-one-click.sh` 生成，安装器会校验其中的版本、源码摘要和 v7/schema 7
清单后才构建两个容器。
...
- 联网协议当前为 v7/schema 7；客户端使用包含战位情报台账、情报分页游标和 VMF 工作流摘要的完整快照建立基线，之后应用带状态版本的连续增量，发现缺少基线、序号缺口或解析失败时自动请求完整同步。v6/v4 仍可按协商结果降级，升级前必须备份房间检查点和事件日志并协调更新客户端与服务器。
- 每次初始场景修改都会清除双方就绪状态。
