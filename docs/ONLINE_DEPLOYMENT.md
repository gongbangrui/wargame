...
当前联网数据面为协议 v8/schema 8。v8 同时支持 `vmf-demo-v2` 演示模式与
`vmf-guided-strike-v1` 兼容模式；演示模式提供六阶段/十动作、固定靶、自动接管、明文消息和
wire trace。普通房间仍可与 v7/v6/v4 客户端降级协商。发布前应协调替换客户端和 `game-server`，并备份 `/data/room-checkpoint.json`、
`/data/room-commands.jsonl` 及轮转日志。旧检查点缺少情报字段时会按空台账兼容恢复，但升级
前仍必须保留可验证的原始备份。

桌面客户端、账号网页和权威服务端必须来自同一个 `release-identity.txt`。在发布机运行
`WARGAME_VERSION=2.0.0 ./tools/build-release.sh --clean` 生成桌面与两种服务端构建；服务器
安装包由 `deploy/package-one-click.sh` 生成，安装器会校验其中的版本、源码摘要和 v8/schema 8
清单后才构建两个容器。
...
- 联网协议当前为 v8/schema 8；客户端使用包含战位情报台账、情报分页游标和 VMF/演示摘要的完整快照建立基线，之后应用带状态版本的连续增量，发现缺少基线、序号缺口或解析失败时自动请求完整同步。普通房间可按协商结果降级到 v7/v6/v4；`vmf-demo-v2` 与 `vmf-guided-strike-v1` 房间只接受 v8/schema 8。升级前必须备份房间检查点和事件日志并协调更新客户端与服务器。演示模式专项恢复和验收见 [`VMF_DEMO_V2.md`](VMF_DEMO_V2.md)。
- 每次初始场景修改都会清除双方就绪状态。
