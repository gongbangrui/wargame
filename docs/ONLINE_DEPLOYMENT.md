...
当前联网数据面为协议 v8/schema 8。v8 客户端在 v7 VMF 工作流之上增加严格 VMF 原子任务、
profile 和 wire trace；普通房间仍可与 v7/v6/v4 客户端降级协商。发布前应协调替换客户端和 `game-server`，并备份 `/data/room-checkpoint.json`、
...
- 联网协议当前为 v8/schema 8；客户端使用包含战位情报台账、情报分页游标和 VMF 工作流摘要的完整快照建立基线，之后应用带状态版本的连续增量，发现缺少基线、序号缺口或解析失败时自动请求完整同步。普通房间可按协商结果降级到 v7/v6/v4；`vmf-guided-strike-v1` 房间只接受 v8/schema 8。升级前必须备份房间检查点和事件日志并协调更新客户端与服务器。
- 每次初始场景修改都会清除双方就绪状态。
