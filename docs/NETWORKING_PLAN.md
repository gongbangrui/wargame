...
本文保留为联网架构的设计背景。仓库现已实现本地模式与权威服务器联网模式：
`server/game/` 是无界面推演服务器，`server/account/` 是账号管理平台，`deploy/`
...
当前发布默认使用 v8/schema 8；普通房间可在协商阶段兼容 v7/v6/v4，但同一连接内不得混用
...
# 终端 3、4：分别启动红蓝客户端
./build/appindex --server ws://127.0.0.1:8080/ws --token "$RED_TOKEN"
./build/appindex --server ws://127.0.0.1:8080/ws --token "$BLUE_TOKEN"
